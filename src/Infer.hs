...
inferExpr :: Env -> CfExpr () -> Either TypeError Scheme
...
infer' :: CfExpr () -> Infer (CfExpr Type)
infer' expr = case unwrap expr of
  Lit (LInt n)  -> return $ typeInt :< Lit (LInt n)
  Lit (LBool b) -> return $ typeBool :< Lit (LBool b)
  Var x -> lookupEnv x >>= \t -> return $ t :< Var x
  Lam x e -> do
    tv <- fresh
    (t :< e') <- inEnv (x, Forall [] tv) (infer' e) 
    return $ tv `TArr` t :< Lam x (t :< e')
  App e1 e2 -> do
    (t1 :< e1') <- infer' e1
    (t2 :< e2') <- infer' e2
    tv <- fresh
...
inferTop :: Env -> [(String, CfExpr ())] -> Either TypeError Env
